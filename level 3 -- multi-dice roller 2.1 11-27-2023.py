...
dice_rolled = False
# The next two lines will enable or disable the ability for the program to remember which type of dice or how many dice you rolled last
...
while True:
    ...
    if button_right.value() == 1 and dice_type_selected == False and dice_number_selected == False and dice_rolled == False:
        ...
        if dice_type == 20 or dice_type == 100:
            dice_quantity = 1
            dice_number_selected = True
            dice_rolled = True
            roll_1d()
# This makes sure that if you have previously selected 9 or 10 as your dice quantity, that it will not carry over into
# allowing you to choose 9 or 10 dice when your dice type is a D10 or D12, which would cause display issues due to
# the character limit of the LCD display.
        elif dice_type < 20:
            if dice_type == 10 and dice_quantity >= 9:
                dice_quantity = 8
            elif dice_type == 12 and dice_quantity >= 9:
                dice_quantity = 8
...
# - lastly, if the dice is a D4, D6, or D8, you can only roll up to 10 of them. This is also due to the display restrictions.
        ...
        if dice_type == dice_12 and dice_quantity == 8:
            dice_quantity = 1
            utime.sleep(.5)
            mylcd.lcd_display_string("       " + str(dice_quantity) + "        ", 2)
            ...
            utime.sleep(.5)
            mylcd.lcd_display_string("       " + str(dice_quantity) + "        ", 2)
#         elif dice_type == 10 and dice_quantity >= 9:
#             dice_quantity = 8
#             utime.sleep(.5)
#             mylcd.lcd_display_string("       " + str(dice_quantity) + "        ", 2)
#         elif dice_type == 12 and dice_quantity >= 9:
